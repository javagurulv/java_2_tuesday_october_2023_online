package lv.javaguru.travel.insurance.rest.v2.person;

import lv.javaguru.travel.insurance.rest.v2.TravelCalculatePremiumControllerV2TestCase;
import org.junit.jupiter.api.DisplayName;
import org.junit.jupiter.api.Test;

public class PersonLevelV2TestCases extends TravelCalculatePremiumControllerV2TestCase {
    @Test
    @DisplayName("ERROR_CODE_1 one personFirstName is NULL, must not be empty")
    public void executeTestCase2() throws Exception {
        executeAndCompare("person/ERROR_CODE_1_personFirstName_is_null");
    }

    @Test
    @DisplayName("ERROR_CODE_1 one personFirstName is empty, must not be empty")
    public void executeTestCase3() throws Exception {
        executeAndCompare("person/ERROR_CODE_1_personFirstName_is_empty");
    }

    @Test
    @DisplayName("ERROR_CODE_1 two personFirstName is empty, must not be empty")
    public void executeTestCase4() throws Exception {
        executeAndCompare("person/ERROR_CODE_1_two_personFirstName_is_empty");
    }

    @Test
    @DisplayName("ERROR_CODE_1 two personFirstName is NULL, must not be empty")
    public void executeTestCase5() throws Exception {
        executeAndCompare("person/ERROR_CODE_1_two_personFirstName_is_null");
    }

    @Test
    @DisplayName("ERROR_CODE_2 one personLastName is NULL, must not be empty")
    public void executeTestCase6() throws Exception {
        executeAndCompare("person/ERROR_CODE_2_personLastName_is_null");
    }

    @Test
    @DisplayName("ERROR_CODE_2 two personLastName is NULL, must not be empty")
    public void executeTestCase7() throws Exception {
        executeAndCompare("person/ERROR_CODE_2_two_personLastName_is_null");
    }

    @Test
    @DisplayName("ERROR_CODE_2 one personLastName is empty, must not be empty")
    public void executeTestCase8() throws Exception {
        executeAndCompare("person/ERROR_CODE_2_personLastName_is_empty");
    }

    @Test
    @DisplayName("ERROR_CODE_2 two personLastName is empty, must not be empty")
    public void executeTestCase9() throws Exception {
        executeAndCompare("person/ERROR_CODE_2_two_personLastName_is_empty");
    }

    @Test
    @DisplayName("ERROR_CODE_11 one personBirthDate is NULL, must not be empty")
    public void executeTestCase10() throws Exception {
        executeAndCompare("person/ERROR_CODE_2_two_personLastName_is_null");
    }

    @Test
    @DisplayName("ERROR_CODE_11 two personBirthDate is NULL, must not be empty")
    public void executeTestCase11() throws Exception {
        executeAndCompare("person/ERROR_CODE_11_personBirthDate_is_null");
    }

    @Test
    @DisplayName("ERROR_CODE_12 one personBirthDate must be in the past")
    public void executeTestCase12() throws Exception {
        executeAndCompare("person/ERROR_CODE_12_personBirthDate_in_the_future");
    }

    @Test
    @DisplayName("ERROR_CODE_12 two personBirthDate must be in the past")
    public void executeTestCase13() throws Exception {
        executeAndCompare("person/ERROR_CODE_12_two_personBirthDate_in_the_future");
    }

    @Test
    @DisplayName("ERROR_CODE_16 one person personCode is NULL, must not be empty")
    public void executeTestCase32() throws Exception {
        executeAndCompare("person/ERROR_CODE_16_personCode_is_null");
    }

    @Test
    @DisplayName("ERROR_CODE_16 one person personCode is empty, must not be empty")
    public void executeTestCase33() throws Exception {
        executeAndCompare("person/ERROR_CODE_16_personCode_is_empty");
    }

    @Test
    @DisplayName("ERROR_CODE_16 two persons personCode is NULL, must not be empty")
    public void executeTestCase34() throws Exception {
        executeAndCompare("person/ERROR_CODE_16_two_personCode_is_null");
    }

    @Test
    @DisplayName("ERROR_CODE_16 two persons personCode is empty, must not be empty")
    public void executeTestCase35() throws Exception {
        executeAndCompare("person/ERROR_CODE_16_two_personCode_is_empty");
    }
}
